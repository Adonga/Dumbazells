...
void main()
{
	...
	if(type == 0) 	   gl_FragColor *= back.x > 0.0 ? luminance : 0.0;
	else if(type == 1) gl_FragColor *= back.y > 0.0 ? luminance : 0.0;
	else if(type == 2) gl_FragColor *= back.z > 0.0 ? luminance : 0.0;
